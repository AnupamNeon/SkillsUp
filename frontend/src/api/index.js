...
// ── Quiz (Educator) ──────────────────────────
export const generateAIQuiz = (data) => api.post("/quiz/generate", data);
export const createManualQuiz = (data) => api.post("/quiz/manual", data);
export const fetchEducatorQuizzes = (courseId, params) =>
  api.get(`/quiz/course/${courseId}`, { params });
...
export const fetchQuizAnalytics = (quizId) =>
  api.get(`/quiz/educator/analytics/${quizId}`);

// ── Quiz (Student) ────────────────────────────
export const fetchCourseQuizzes = (courseId, params) =>
  api.get(`/quiz/course/${courseId}`, { params });
export const fetchQuizForStudent = (quizId) => api.get(`/quiz/${quizId}`);
export const submitQuizAnswers = (quizId, data) =>
  api.post(`/quiz/${quizId}/submit`, data);
export const fetchQuizResults = (quizId, attemptId) =>
  api.get(`/quiz/${quizId}/results`, { params: { attemptId } });
export const fetchMyQuizAttempts = (courseId, params) =>
  api.get(`/quiz/my-attempts/${courseId}`, { params });

// ── Content Check & Lecture Descriptions (NEW) ──
export const checkChapterContent = (data) =>
  api.post("/quiz/check-content", data);
export const updateLectureDescriptions = (courseId, descriptions) =>
  api.put(`/educator/courses/${courseId}/lecture-descriptions`, {
    descriptions,
  });
